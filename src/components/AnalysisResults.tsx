...
const AnalysisResults = ({ analysis, userPhoto, onReset }: AnalysisResultsProps) => {
  ...
  // Normalize landmarks - support both old (eyePositions) and new (facialLandmarks) format
  const facialLandmarks: FacialLandmarks | undefined = useMemo(() => {
    if (analysis.facialLandmarks) {
      return analysis.facialLandmarks;
    }
    if (analysis.eyePositions) {
      return {
        leftEye: analysis.eyePositions.leftEye,
        rightEye: analysis.eyePositions.rightEye,
      };
    }
    return undefined;
          ...
          {glassesCatalog.map((style) => (
            ...
            >
              ...
              <Button 
                variant="cta" 
                size="sm" 
                className="mt-3 w-full animate-pulse-glow flex items-center justify-center gap-2"
                onClick={(e) => {
                  e.stopPropagation();
